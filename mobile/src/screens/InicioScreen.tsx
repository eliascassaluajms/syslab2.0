...
export default function InicioScreen({ navigation }: Props) {
  ...
  const nombre = usuario ? `${usuario.nombre} ${usuario.apellido}` : 'Estudiante';
    ...
    <ScrollView style={estilos.pantalla} contentContainerStyle={estilos.contenido}>
      <View style={estilos.perfil}>
        ...
        <View style={estilos.perfilInfo}>
          ...
          <Text style={estilos.ru}>RU: {usuario?.username ?? '—'}</Text>
        </View>
      ...
      <View style={estilos.opciones}>
        <CardAccion
          icono="🗓️"
          titulo="Mi horario"
          descripcion="Consulta las materias, laboratorios y horarios de tu gestión."
          onPress={() => navigation.navigate('MiHorario')}
        />
        <CardAccion
          icono="✅"
          titulo="Marcar asistencia"
          descripcion="Escanea el código QR de la sesión en el laboratorio."
          onPress={() => navigation.navigate('MarcarAsistencia')}
        />
        <CardAccion
          icono="🔓"
          titulo="Desbloquear equipo"
          descripcion="Ingresa el código de 2 dígitos o escanea el QR de la PC."
          onPress={() => navigation.navigate('Desbloquear')}
        />
      </View>
