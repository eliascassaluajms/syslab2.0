...
import { ETIQUETAS_ESTADO } from '../../views/docente/incidencias.constantes';
...
const ESTADOS_GESTION: EstadoIncidencia[] = ['PENDIENTE', 'EN_REVISION', 'EN_PROCESO', 'RESUELTO', 'DESCARTADO'];
...
export const ModalGestionIncidencia: React.FC<Props> = ({ isOpen, onClose, incidencia, onGuardar }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (guardando) return;
    if (estado === 'RESUELTO' && !solucion.trim()) {
      alert('Para marcar como "Resuelto", registre la solución aplicada.');
      return;
    }
    setGuardando(true);
    try {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 text-slate-100 shadow-2xl space-y-4">
        <div className="border-b border-slate-800 pb-3">
          ...
          <p className="text-xs text-slate-400 mt-1 font-mono">{incidencia.folio || `Ticket #${incidencia.id}`}: {incidencia.titulo}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-3 border-t border-slate-800">
            ...
            <button
              type="submit"
              disabled={guardando}
              className="px-4 py-2 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-500 disabled:opacity-50 rounded-xl transition-all shadow-lg shadow-blue-950/40 cursor-pointer"
            >
